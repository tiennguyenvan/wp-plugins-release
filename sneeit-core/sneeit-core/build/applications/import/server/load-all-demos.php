...
function sneeit_core_load_all_demos() {
	...
	if ( is_wp_error( $sneeit_core_lad_post ) ) {
		// dev-reply#625.
		sneeit_core_ajax_error_die( 'Error fetching JSON data: ' . $sneeit_core_lad_post->get_error_message() . ' (' . $sneeit_core_lad_url . ')' );
		// dev-reply#629.
	}
	...
	if ( ! $sneeit_core_lad_body ) {
		sneeit_core_ajax_error_die( 'Loading all demos failed: Invalid JSON data' . $sneeit_core_lad_url );
	}
	...
	foreach ( $sneeit_core_lad_body as $sneeit_core_lad_list => $sneeit_core_lad_demo ) {
		...
		$sneeit_core_lad_categories = is_numeric( $sneeit_core_lad_list ) ? "" : $sneeit_core_lad_demo; // dev-reply#643.
...
/**
 * Check Documentation#641
 *
 * @param object|array|string $sneeit_core_lad_file check var-def#641.
 */
