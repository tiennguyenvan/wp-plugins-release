...
function sneeit_core_load_selected_demo_content() {
	...
	if ( is_wp_error( $sneeit_core_lsdc_post ) ) {
		sneeit_core_ajax_error_die( 'Error fetching demo JSON data: ' . $sneeit_core_lsdc_post->get_error_message() . ' (' . $sneeit_core_lsdc_jsonurl . ')' );
	}
	...
	if ( ! $sneeit_core_lsdc_body ) {
		sneeit_core_ajax_error_die( 'Loading selected demo failed: Invalid JSON data' );
	}
